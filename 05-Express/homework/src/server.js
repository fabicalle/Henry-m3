...
server.get("/posts", (req, res) => {
    ...
    if (term) {
        const result = posts.filter(p => { return p.title.includes(term) || p.contents.includes(term) });
...
server.get("/posts/:author/:title", (req, res) => {
    ...
    const result = posts.filter(p => { return p.author === author && p.title === title });
    if (result.length) {
        res.status(200).json(result);
    } else {
...
server.put("/posts", (req, res) => {
    ...
    if (post) {
       ...
       res.status(200).json(post);
        ...
        res.status(STATUS_USER_ERROR).json({ error: "No se encuentra el id buscado" });
    }
